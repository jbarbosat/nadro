...
#      
  ...
  output$PrecioBase1 <- renderPrint({
    if(input$ifEscenarios == TRUE && input$material != 'Todos'){
      pb<-mean(vt.vector)*1000000/18500
      print(paste(c("Precio promedio: ", round(pb,2)),collapse=''))
    }
  })
  
  output$EscVentasPlot <- renderPlot({
    if(input$ifEscenarios == TRUE && input$material != 'Todos'){
      #vt.vector<-na.omit(unlist(c(v.tabla[1,-1],v.tabla[2,-1],v.tabla[3,-1])))
      leidos<-c(input$p1,input$p2,input$p3,input$p4,input$p5,input$p6)
      ventasnuevas<-mean(vt.vector)*(1-leidos/100)
       vt.serie2<-ts(c(vt.vector[length(vt.vector)],ventasnuevas),frequency=12, start=c(2013,7))
       ts.plot(vt.serie,vt.serie2,gpars=list(ylab="Millones de pesos",xlab="Tiempo",tck=1,col=c("red","blue"),lwd=2))
    }
  ...
  output$CostosM1Plot<-renderPlot({ 
    if(input$ifEscenarios == TRUE && input$material != 'Todos'){
      leidos<-c(input$p1,input$p2,input$p3,input$p4,input$p5,input$p6)
      costosnuevas<-mean(cm1.vector)*(1-leidos/100)
      cm1.serie2<-ts(c(cm1.vector[length(cm1.vector)],costosnuevas),frequency=12, start=c(2013,7))
      ts.plot(cm1.serie,cm1.serie2,gpars=list(ylab="Millones de pesos",main="Escenarios de Precios",
                                              xlab="Tiempo",tck=1,col=c("red","blue"),lwd=2))
    }
    else{
  ...
  output$CostosM2Plot<-renderPlot({ 
    if(input$ifEscenarios == TRUE && input$material != 'Todos'){
      leidos<-c(input$p1,input$p2,input$p3,input$p4,input$p5,input$p6)
      costosnuevas<-mean(cm2.vector)*(1-leidos/100)
      cm2.serie2<-ts(c(cm2.vector[length(cm2.vector)],costosnuevas),frequency=12, start=c(2013,7))
      ts.plot(cm2.serie,cm2.serie2,gpars=list(ylab="Millones de pesos",main="Escenarios de Precios",
                                              xlab="Tiempo",tck=1,col=c("red","blue"),lwd=2))
    }
    else{
  ...
  #24    ZPG5  -97403.57  SDPromoNad Gral Fact
    ...
    d.columna<-c(3,4)
    d.pre.tabla2 <- reshape(agg1[,c(1,2,d.columna)], idvar = "FactorA", timevar = "FactorB", direction = "wide")
    d.pre.tabla3<-as.data.frame(t(d.pre.tabla2))
    d.indices<-as.numeric(d.pre.tabla2[,1])
    d.tabla<-d.pre.tabla3[-1,d.indices]
    d.Year<-row.names(d.tabla)
    d.tabla<-cbind(d.Year,d.tabla)
    names(d.tabla)<-c("Year","Ene","Feb","Mar","Abr","May","Jun","Jul","Ago","Sep","Oct","Nov","Dic")
    #tabla$Mes<-c("Ene","Feb","Mar","Abr","May","Jun","Jul","Ago","Sep","Oct","Nov","Dic")
  ...
  output$DescM1Plot<-renderPlot({ 
    if(input$ifEscenarios == TRUE && input$material != 'Todos'){
      leidos<-c(input$p1,input$p2,input$p3,input$p4,input$p5,input$p6)
      descnuevas<-mean(dm1.vector)*(1-leidos/100)
      dm1.serie2<-ts(c(dm1.vector[length(dm1.vector)],descnuevas),frequency=12, start=c(2013,7))
      ts.plot(dm1.serie,dm1.serie2,gpars=list(ylab="Millones de pesos",main="Escenarios de Precios ZDFI",
                                              xlab="Tiempo",tck=1,col=c("red","blue"),lwd=2))
    }
    else{
    ...
    dm2.vector<-as.double(na.omit(unlist(c(d.tabla[2,-1],d.tabla[4,-1],d.tabla[6,-1]))))/1000
  ...
  output$DescM2Plot<-renderPlot({  
    if(input$ifEscenarios == TRUE && input$material != 'Todos'){
      leidos<-c(input$p1,input$p2,input$p3,input$p4,input$p5,input$p6)
      descnuevas<-mean(dm2.vector)*(1-leidos/100)
      dm2.serie2<-ts(c(dm2.vector[length(dm2.vector)],descnuevas),frequency=12, start=c(2013,7))
      ts.plot(dm2.serie,dm2.serie2,gpars=list(ylab="Millones de pesos", main="Escenarios de Precios ZPG5",
                                              xlab="Tiempo",tck=1,col=c("red","blue"),lwd=2))
    }
    else{
  ...
  output$RentPlot<-renderPlot({
    if(input$ifEscenarios == TRUE && input$material != 'Todos'){
      leidos<-c(input$p1,input$p2,input$p3,input$p4,input$p5,input$p6)
      ventasnuevas<-mean(vt.vector)*(1-leidos/100)
      vt.serie3<-ts(c(vt.vector,ventasnuevas),frequency=12, start=c(2013,7))
      descnuevas<-mean(dm1.vector)*(1-leidos/100)
      dm1.serie3<-ts(c(dm1.vector,descnuevas),frequency=12, start=c(2013,7))
      costosnuevas<-mean(cm2.vector)*(1-leidos/100)
      cm2.serie3<-ts(c(cm2.vector,costosnuevas),frequency=12, start=c(2013,7))
      ts.plot(vt.serie3-cm2.serie3-dm1.serie3,gpars=list(tck=1),col=c("red"),lwd=2,main="Rentabilidad",ylab="Tiempo")
    }
    else{
      ts.plot(vt.serie-cm2.serie-dm1.serie,gpars=list(tck=1),col=c("red"),lwd=2,main="Rentabilidad",ylab="Tiempo")
    }
    abline(h=1,lwd=2)
  })
  ...
  output$TopProducto<-renderPlot({
    ...
    barplot(producto, main="Top 10 Productos", horiz=TRUE,
            col=gray(0:10/10),
            names.arg=c("LACTACYD","VARTALON","ASPIRINA-PROTEC","CELEBREX ","DOLO-NEUROBION FORTE", "EVRA","CIALIS","ULSEN","KRYTANTEK","MICARDIS PLUS"), cex.names=0.7,las=1,xlim=c(0,25))
  })
 ...
 output$DescTopCl<-renderPlot({
    ...
    symbols(x=dfx$ev1, y=dfx$ev2, circles=dfx$ev3, inches=1/3, main="Descuentos vs Ventas",ylab="Ventas (MDP)", xlab=NA, bg=rainbow(5), fg=NULL, xaxt = 'n',cex.axis=.8)
 ...
 output$Reglas<-renderPlot({
  ...
  output$IdMatMB<-renderTable({
    ...
    info<-c("Antiinflamatorio, analgesico",
            ...
            "Antipiretico",
            "Antiinflamatorio, analgesico")
    mb.tabla<-as.table(cbind(ids,nombres,info))
    row.names(mb.tabla)<-NULL
    print(mb.tabla,justify="left") 
  })
